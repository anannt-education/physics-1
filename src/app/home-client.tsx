...
import { PUBLIC_LESSONS, whatsappUrl } from "@/lib/mount";
    ...
    <div className="space-y-8">
      <header>
        ...
        <h1 className="mt-1 font-heading text-3xl tracking-tight sm:text-4xl">
          {named
            ...
            : "Two Unit 1 motion lessons. Graph-reading first. Units 2–8 are unpublished."}
        </h1>
        <p className="mt-2 max-w-2xl text-muted-foreground">
          {named
            ...
            : "Anannt Education coaches the May 2027 exam the way a careful teacher would: find the stuck idea, repair graph reading when height is treated as slope. A self-study supplement — this desk does not predict an official AP score."}
        </p>
        {!named ? (
          <div className="mt-4 flex flex-wrap gap-2">
            <Button nativeButton={false} render={<Link href={PUBLIC_LESSONS[0].path} />}>
              Start lesson 1 — motion graphs
            </Button>
            <Button variant="outline" nativeButton={false} render={<Link href={PUBLIC_LESSONS[1].path} />}>
      ...
      {!named ? (
        <p className="text-sm text-muted-foreground">
          ...
          . Graph-repair paths stay open. A person in Burjuman can sit with a stuck graph after two
          lessons —{" "}
          <a href={whatsappUrl("doubts")} className="underline underline-offset-2">
            WhatsApp
          </a>
          .
        </p>
        ...
        <Card>
          ...
          <CardContent className="text-sm">
            ...
            <p className="mt-2 text-muted-foreground">
              Full mocks are not in this slice. Units 2–8 are unpublished. This studio does not
              predict an official AP score.
            </p>
          </CardContent>
        </Card>
      </div>
